...
export function ReviewStep({ cart, goTo, next }: { cart: BookingCart; goTo: (step: number) => void; next: () => void }) {
  ...
  return (
    <>
      <ScreenShell>
        <StepHeader eyebrow="Step 06" title="Review the split." sub="The operator's rental charge and the Exotiq booking fee + protection are separate charges, broken out before checkout." />
        ...
        <Breakdown title="Exotiq.Rent" note="Booking fee + protection, charged separately by EXOTIQ.RENT" rows={[[`Booking fee (${platformPercent}%)`, `On ${formatMoney(cart.totals.platformFeeBaseCents)} rental only; extras & deposit excluded`, cart.totals.platformFeeCents], ['Protection plan', cart.protection === 'decline' ? 'Declined — hold required later' : `${cart.protection} · ${cart.totals.days} days`, cart.totals.protectionTotalCents, () => goTo(4)]]} total={cart.totals.exotiqTotalCents} />
