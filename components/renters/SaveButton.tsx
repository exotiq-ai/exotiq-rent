 ...
 */
export function SaveButton({ car, className = '', size = 16 }: { car: Omit<SavedCar, 'savedAt'>; className?: string; size?: number }) {
  ...
  return (
    ...
    >
      <Heart size={size} strokeWidth={1.75} className={`${saved ? 'fill-[#C8A664] text-[#C8A664]' : ''} ${bounce ? 'animate-heart-bounce' : ''}`} onAnimationEnd={() => setBounce(false)} aria-hidden />
    </button>
  );
}
