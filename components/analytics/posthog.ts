 ...
 * unit-tested without rendering; PostHogInit mounts it. `person_profiles:
 * 'identified_only'` keeps anonymous browsing anonymous — renters are never
 * identified by this app (no identify() call exists), so profiles stay off.
 *
 * `sanitize_properties` strips the confirmation access token (`?t=`) from
 * every URL property PostHog records ($current_url, $referrer and their
 * $initial_ variants): a pageview of /booking/BK-…?t=… must not hand the
 * booking's credential to a third party.
 */
...
posthog.init(${JSON.stringify(key)},{api_host:${JSON.stringify(host)},person_profiles:'identified_only',capture_pageview:true,capture_pageleave:true,disable_session_recording:true,sanitize_properties:function(p){for(var k of ['$current_url','$referrer','$initial_current_url','$initial_referrer','$pathname']){if(typeof p[k]==='string'){p[k]=p[k].replace(/([?&])t=[^&#]*/g,'$1t=redacted')}}return p}});`;
}
