...
export type CaptureRequest = {
  ...
  /** Explicit marketing opt-in. False still records the person (a saved list or an alert is a service they asked for). */
  consent: boolean;
  path?: string;
  booking_ref?: string;
  team_slug?: string;
...
export function validateCapture(body: unknown, today: string = todayIso()): ValidationResult {
  ...
  // Honeypot: real forms never fill it.
  if (typeof b.website === 'string' && b.website.trim() !== '') return { ok: false, error: 'Rejected.' };
  ...
  if (b.alert !== undefined) {
    ...
    if (start < today) return { ok: false, error: 'Those dates have passed.' };
  ...
  return { ok: true, value: { email, name, phone, source, consent, path, booking_ref, team_slug, vehicle_slug, saved, alert } };
}
