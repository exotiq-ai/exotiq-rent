...
export default function ConfirmedPage({ searchParams }: { searchParams?: { state?: string; sent?: string; marketing?: string } }) {
  ...
  return (
    <BrowseChrome view={null}>
      <section className={`${containerClassName} max-w-2xl pb-24 pt-16 sm:pt-24`}>
        ...
        <p className="mt-5 max-w-xl text-[15px] leading-7 text-[#9BA1B0]">{copy.body}{sentList ? ' Your saved cars are on their way to your inbox.' : ''}{raw === 'ok' ? ' Any availability alert you set is now on: we check every morning and e-mail you once when the dates open up.' : ''}</p>
